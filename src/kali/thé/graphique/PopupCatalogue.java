    ...
    public void actionPerformed(ActionEvent e) {
        for (int i=0; i<j.tabTheButton.length; i++){
            if (e.getSource() == j.tabTheButton[i]){
                ret = new The(j.tabThe.get(i).getTempsInfusion(),j.tabThe.get(i).getNom(),j.tabThe.get(i).getTemperature(),j.tabThe.get(i).getDescription(),j.tabThe.get(i).getCategorie());
